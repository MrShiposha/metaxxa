...
#define DECLARE_FUNCTION_WITH_ARGUMENTS(Signature) 										                   					\
	...
	{																					                   					\
		...
		using ToMethod = typename detail::CallableToMethod<SomeType, Signature, std::tuple_size_v<Arguments> == 0>::Method; \
																															\
		template <typename SomeType>																						\
...
#define DECLARE_METHOD_WITH_ARGUMENTS(Signature) \
	...
	{																														\
		...
		using ToMethod = typename detail::CallableToMethod<RHSType, Signature, std::tuple_size_v<Arguments> == 0>::Method;  \
																															\
		template <typename RHSType>																						    \
